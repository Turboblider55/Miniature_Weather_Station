...
esp_err_t ens160_aht21_init(ens160_aht21_handle_t *handle, i2c_master_bus_handle_t bus_handle)
{
    ...
    // For this example, we will proceed even if ENS160 is not detected, since AHT21 can still provide temperature and humidity data
    ESP_LOGI(TAG, "AHT21 initialized successfully (ENS160 disabled)");
    return ESP_OK;
...
// }
    ...
    // Read all sensor data (temperature, humidity, TVOC, eCO2)
    // For this example, we will read AHT21 data and return dummy values for ENS160 since it is disabled
    // esp_err_t ens160_aht21_read_all_data(
    //     ens160_aht21_handle_t *handle,
    //     float *temperature,
    //     float *humidity,
    //     uint16_t *tvoc,
    //     uint16_t *eco2)
    ...
    //     Read AHT21 data only
    //     ret = aht21_read_data(handle, temperature, humidity);
    //     if (ret != ESP_OK) {
    //         return ret;
    //     }

    //     ENS160 disabled → return dummy values
    //     *tvoc = 0;
    //     *eco2 = 0;
    ...
    // Updated version of read_all_data that also includes dummy pressure and altitude values for display testing
    esp_err_t ens160_aht21_read_all_data(
        ens160_aht21_handle_t *handle,
        float *temperature,
        float *humidity)
    {
        ...
        // ENS160 disabled → return dummy values for TVOC and eCO2
        // *tvoc = 0;   
        // *eco2 = 0;
        // *pressure = 1013.25f;  // Standard atmospheric pressure in hPa
        // *altitude = 0.0f;      // Sea level altitude

         // For testing, we can return some dummy values that change over time
        return ESP_OK;
    }
